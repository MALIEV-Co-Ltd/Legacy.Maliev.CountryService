...
public sealed class DependabotConfigurationContractTests
{
    ...
    public void NuGetUpdater_ScansOnlyIndependentlyResolvableProjectDirectories()
    {
        ...
        foreach (var directory in new[]
                 {
            "/Legacy.Maliev.CountryService.Application",
            "/Legacy.Maliev.CountryService.Data",
            "/Legacy.Maliev.CountryService.Domain",
                 })
        {
            Assert.Contains($"      - {directory}", source, StringComparison.Ordinal);
        }

        Assert.Equal(3, source.Split("\n      - /Legacy.Maliev.CountryService.", StringSplitOptions.None).Length - 1);
    }

    [Fact]
    public void NuGetUpdater_DoesNotIgnoreCoordinatedEfAndNpgsqlRuntimeGraph()
    {
        var source = ReadNuGetBlock();
